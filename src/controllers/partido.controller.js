    ...
    if (req.user.rol == "Usuario") {
        ...
        if (req.params.idUsuario == null) {
            return res.status(500)
                    .send({ mensaje: "debe enviar el id del usuario al que quiere crear el partido",});
        }
                                                        ...
                                                        Partido.findOne({ idEquipo2: idEquipo2, jornada: parametros.jornada }, (err, partidoEncontrado2) => {
                                                            if (!partidoEncontrado2) {
                                                                ...
                                                                partidoModel.save((err, partidoCreado) => {
                                                                    ...
                                                                    if (!partidoCreado)
                                                                        return res.status(500)
                                                                                .send({ mensaje: "Hubo un error al crear un nuevo partido:(" });
                                                                ...
                                                                return res.status(500)
                                                                            .send({ error: "el equipo 2 ya jugó en esta jornada" })
                                                            }
                                                        })
                                                    } else {
                                                        return res.status(500)
                                                                                .send({ error: "el equipo 1 ya jugó en esta jornada" })
                                                    }
                                                })
                                            } else {
                                                return res.status(500)
                                                                                        .send({ error: "El equipo ingresado no existe en la jornada" })
                                            }
                                        })
                                    } else {
                                        return res.status(500)
                                                    .send({ error: "El equipo no existe" })
                                    }
                                })
                            } else {
                                return res.status(500)
                                                        .send({ error: "La jornada se encuentra sin cupo" })
                            }
                        })
                    } else {
                        return res.status(500)
                                                            .send({ error: "Valores de la jornada invalidos" })
                    }
            
                } else {
                    return res.status(500)
                                                                    .send({ error: "Todos los datos deben de estar llenos UnU" })
                }
